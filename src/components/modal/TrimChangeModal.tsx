...
export default function TrimChangeModal({colorName, setModal, data}:{colorName: string, setModal: any, data:TrimChangeModalDataType}) {
	...
	return (<Modal setModal={setModal}>
		<div>
			<PopupHeader><h3>{colorName}색상은 트림 변경 후 선택 가능합니다.</h3></PopupHeader>
			<div>
				<p>트림을 변경하시겠습니까?</p>
				<TrimWrap>
					<FlexTrim>
						<FlexLiItem>
							<b>현재 트림</b>
							<TrimBox trimName={data.changeTrimInfo?.beforeTrimName || 'null'} price={data.changeTrimInfo?.beforeCarPrice || 0}></TrimBox>
						</FlexLiItem>
						<FlexLiItem>
							<b>변경 트림</b>
							<TrimBox 
					...
					</FlexTrim>
					{
						...
						<div>
							<p style={{marginBottom:"20px"}}>변경 시 선택 추가되는 품목</p>
							<ul>
								{
								data.changeOptionInfo?.addOptions.map((item, id) => {
									return (<OptionList>
					...
					}
					{
						...
						<div>
							<p style={{marginBottom:"20px"}}>변경 시 선택 해제되는 품목</p>
					...
					<FlexDiv style={{justifyContent: "space-between", marginTop: "20px"}}>
						<p>변경 금액</p>
						<p>{data.changeTrimInfo?.changeCarPrice && data.changeTrimInfo?.changeCarPrice - data.changeTrimInfo.beforeCarPrice + addPrice - delPrice}</p>
					</FlexDiv>
				</TrimWrap>
			</div>
		</div>
