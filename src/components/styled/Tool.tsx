...
const ToolLabel = styled.label`
	...
	font-size: 14px;
`
const RadioInput = styled.input<{type: string, value: number, onClick: any, disabled: string}>`
	opacity: 0;
	outline: none;
	position: absolute;
	z-index: -1;
...
function Tool({tools, toolId, onChange, name}: Props) {
	const listItems = tools?.map((tool, i) => {
		if (toolId === 0 && i === 0 && tool.isSelect === true)
			onChange(tool.id);
		...
		<ToolLabel>
			...
			<RadioInner isSelect={tool.isSelect} choice={Number(tool.id) === Number(toolId)}>{tool.name}</RadioInner>
		</ToolLabel>
		)
	});
	return (
